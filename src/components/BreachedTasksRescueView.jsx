...
export default function BreachedTasksRescueView({ tasks, onCloseRescue, userProfile }) {
  ...
  if (breachedTasks.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-neutral-50 dark:bg-[#0a0a0a] text-neutral-900 dark:text-white transition-colors duration-300">
        ...
        <h2 className="text-3xl font-headings font-bold mb-2 bg-gradient-to-r from-[#00CFCF] to-teal-500 dark:to-white bg-clip-text text-transparent">You're All Caught Up!</h2>
        <p className="text-neutral-500 dark:text-gray-400 mb-8">No tasks have breached their safety buffers.</p>
        <button 
  ...
  return (
    <div className="flex-1 flex bg-white dark:bg-[#0a0a0a] text-neutral-900 dark:text-white overflow-hidden h-full relative transition-colors duration-300">
      ...
      <div className={`flex-1 flex flex-col p-8 transition-all duration-500 ease-in-out ${isAIFullScreen ? 'hidden' : (showAIAssistant ? 'md:pr-4 md:w-1/2' : 'w-full')}`}>
        <div className="flex items-center gap-5 mb-10 border-b border-white/5 pb-6">
          ...
          <div>
            <h1 className="text-4xl font-headings font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-orange-500 dark:from-red-500 dark:to-orange-500 tracking-tight">Rescue Mode</h1>
            <p className="text-neutral-500 dark:text-gray-400 mt-1 font-medium flex items-center gap-2">
            ...
            <div 
              ...
              className={`p-6 rounded-2xl border transition-all duration-300 cursor-pointer group relative overflow-hidden ${
                selectedTask?.id === task.id 
                  ? 'bg-red-50 dark:bg-gradient-to-br dark:from-red-900/40 dark:to-black/80 border-red-500/50 scale-[1.02] shadow-2xl shadow-red-900/10 dark:shadow-red-900/20' 
                  : 'bg-white dark:bg-white/[0.02] border-neutral-200 dark:border-white/10 hover:bg-neutral-50 dark:hover:bg-white/[0.04] hover:border-red-500/30'
              }`}
            >
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1 pr-4">
                  <h3 className={`text-xl font-bold mb-3 transition-colors ${selectedTask?.id === task.id ? 'text-neutral-900 dark:text-white' : 'text-neutral-800 dark:text-neutral-300 group-hover:text-red-500 dark:group-hover:text-red-400'}`}>
                    {task.title}
                  </h3>
                  <div className="flex items-center gap-4 text-sm text-neutral-500 dark:text-gray-400">
                    <span className="flex items-center gap-1.5 bg-neutral-100 dark:bg-black/40 px-2.5 py-1 rounded-md border border-neutral-200 dark:border-white/5">
              ...
              <div className="mt-5 pt-4 border-t border-neutral-200 dark:border-white/5 flex justify-between items-center">
                <p className="text-xs text-red-500 dark:text-red-400/80 font-bold uppercase tracking-widest flex items-center gap-1.5">
      ...
      {showAIAssistant && selectedTask && (
        <div className={`border-l border-neutral-200 dark:border-white/10 bg-white/95 dark:bg-[#141414]/95 backdrop-blur-3xl shadow-2xl transition-all duration-500 ease-in-out flex flex-col h-full ${
          ...
          <div className="p-5 border-b border-neutral-200 dark:border-white/10 flex justify-between items-center bg-neutral-50 dark:bg-black/20">
            <div>
              <h2 className="font-bold text-lg flex items-center gap-2.5 text-neutral-900 dark:text-white">
                <div className="p-1.5 bg-[#FF6A00]/10 dark:bg-[#FF6A00]/20 rounded-lg">
                  <Sparkles className="w-5 h-5 text-[#FF6A00]" />
                </div>
                Laila Rescue Assistant
              </h2>
              <p className="text-xs text-neutral-500 dark:text-gray-400 mt-1 truncate max-w-[400px]">Focusing on: <span className="text-neutral-900 dark:text-gray-200 font-medium">{selectedTask.title}</span></p>
            </div>
            <div className="flex items-center gap-2">
              <button 
                onClick={toggleFullScreen}
                className="p-2.5 hover:bg-neutral-100 dark:hover:bg-white/10 rounded-xl transition-colors text-neutral-500 dark:text-gray-400 hover:text-neutral-900 dark:hover:text-white"
              ...
              <button 
                ...
                className="p-2.5 hover:bg-red-500/10 dark:hover:bg-red-500/20 hover:text-red-500 rounded-xl transition-colors text-neutral-500 dark:text-gray-400"
                title="Close AI"
              >
